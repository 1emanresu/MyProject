...
<%@page import="com.pay.wapalipay.server.util.AlipayNotify"%>
<%
/* *
 功能：支付宝服务器异步通知页面
 版本：3.3
	...
	if(AlipayNotify.verifyNotify(params)){//验证成功
		...
		if(trade_status.equals("TRADE_FINISHED")){
			//判断该笔订单是否在商户网站中已经做过处理
				...
				//如果有做过处理，不执行商户的业务程序
			YeepayService.service.netcallback(trade_no, out_trade_no, "", notify_time, notify_time);
			YeepayService.service.asynchronous(out_trade_no);
			//注意：
			//该种交易状态只在两种情况下出现
			//1、开通了普通即时到账，买家付款成功后。
			...
			YeepayService.service.failure(out_trade_no, notify_time);
			response.sendRedirect(basePath+"/yeepay/yeeReturn?r6_Order="+out_trade_no);
			out.println("success");
		}
