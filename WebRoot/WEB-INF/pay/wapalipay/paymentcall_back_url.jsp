	...
	if(verify_result){//验证成功
		...
		if(result.equals("success")){
			//判断该笔订单是否在商户网站中已经做过处理
				...
				//如果有做过处理，不执行商户的业务程序
			PaymentService.service.netcallback(trade_no, out_trade_no, "", sdf.format(new Date()), sdf.format(new Date()));
			response.sendRedirect(basePath+"/payment/showPayOk");
		}else{
			PaymentService.service.failure(out_trade_no, sdf.format(new Date()));
