...
$("#partici option[value='${rechparticipate}']").attr("selected", true);
$("#rechstate option[value='${rechstate}']").attr("selected", true);
</script>
</head>
<body>
	<form id="pagerForm" method="post" action="<%=adminbasePath%>/person/showRecharge" onsubmit="return divSearch(this, 'jbsxBox');">
	...
	<div class="pageHeader">
		<form rel="pagerForm" action="<%=adminbasePath%>/person/showRecharge"
			...
			<div class="searchBar">
				<ul class="searchContent">
					<li style="width: 400px"><label>日期：</label><input value="${rechstarttime}" type="text" name="rechstarttime" class="date" dateFmt="yyyy-MM-dd"/>
					--<input value="${rechendtime }" type="text" name="rechendtime" class="date" dateFmt="yyyy-MM-dd"/>
					</li>
					<li style="width: 250px"><label>支付方式：</label>
						<select class="combox" name="rechparticipate" id="partici">
					...
					<li style="width: 200px"><label>订单状态：</label>
						<select class="combox" name="rechstate" id="rechstate" style="width: 100px">
							<option value="0">请选择</option>
			...
			<tbody>
				<c:forEach items="${orderlist}" var="order">
					<tr target="rechargeid" rel="${order.rechargeid }">
						...
						<td>
						<c:if test="${order.r1_Code==1}">
							成功
						</c:if>
						<c:if test="${order.r1_Code==2}">
							处理中
						</c:if>
						<c:if test="${order.r1_Code==3}">
							失败
						</c:if></td>
